...
status_t IommuImpl::Create(fbl::unique_ptr<const uint8_t[]> desc_bytes, uint32_t desc_len,
...
status_t IommuImpl::ValidateIommuDesc(const fbl::unique_ptr<const uint8_t[]>& desc_bytes,
                                      uint32_t desc_len) {
    auto desc = reinterpret_cast<const zx_iommu_desc_intel_t*>(desc_bytes.get());
...
status_t IommuImpl::WaitForValueLocked(RegType* reg,
    ...
    while (true) {
        ...
        lk_time_t sleep_deadline = fbl::min(now + kMaxSleepDuration, deadline);
