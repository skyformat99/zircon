...
zx_status_t sys_iommu_create(zx_handle_t rsrc_handle, uint32_t type, user_ptr<const void> desc,
                             uint32_t desc_len, user_ptr<zx_handle_t> out) {
...
zx_status_t sys_bti_create(zx_handle_t iommu, uint64_t bti_id, user_ptr<zx_handle_t> out) {
    auto up = ProcessDispatcher::GetCurrent();
...
zx_status_t sys_bti_unpin(zx_handle_t bti, user_ptr<const uint64_t> extents, uint32_t extents_len) {
    auto up = ProcessDispatcher::GetCurrent();
