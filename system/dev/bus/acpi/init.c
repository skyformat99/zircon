...
static zx_status_t find_iommus(void);
...
ACPI_STATUS init(void) {
    ...
    zx_status_t zx_status = find_iommus();
    if (zx_status != ZX_OK) {
...
static zx_status_t acpi_scope_to_desc(ACPI_DMAR_DEVICE_SCOPE* acpi_scope,
    ...
    switch (acpi_scope->EntryType) {
        ...
        case ACPI_DMAR_SCOPE_TYPE_BRIDGE:
            desc_scope->type = ZX_IOMMU_INTEL_SCOPE_BRIDGE;
            break;
        default:
...
static zx_status_t create_non_whole_segment_iommu_desc(ACPI_TABLE_DMAR* table,
                                                       ACPI_DMAR_HARDWARE_UNIT* unit,
                                                       zx_iommu_desc_intel_t** desc_out,
                                                       size_t* desc_len_out) {
...
static zx_status_t find_iommus(void) {
    ...
    uintptr_t addr;
    zx_handle_t iommu_handle = ZX_HANDLE_INVALID;
    for (addr = records_start; addr < records_end;) {
        ACPI_DMAR_HEADER* record_hdr = (ACPI_DMAR_HEADER*)addr;
        ...
        switch (record_hdr->Type) {
            case ACPI_DMAR_TYPE_HARDWARE_UNIT: {
                ...
                if (whole_segment) {
                    zx_status = create_whole_segment_iommu_desc(dmar, rec, &desc, &desc_len);
                } else {
                    zx_status = create_non_whole_segment_iommu_desc(dmar, rec, &desc, &desc_len);
                }
                if (zx_status != ZX_OK) {
                ...
                zx_status = zx_iommu_create(root_resource_handle, ZX_IOMMU_TYPE_INTEL,
