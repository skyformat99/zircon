...
static i2c_txn_t* get_i2c_txn(platform_bus_t* bus, pdev_i2c_req_t* req, zx_handle_t channel) {
    ...
    txn->bus = bus;
    txn->complete_cb = req->txn_ctx.complete_cb;
    txn->cookie = req->txn_ctx.cookie;
    txn->channel = channel;
...
static zx_status_t platform_i2c_transact(platform_dev_t* dev, pdev_i2c_req_t* req,
                                      pdev_i2c_resp_t* resp, uint8_t* data, zx_handle_t channel) {
    i2c_channel_t* i2c_channel = (i2c_channel_t *)req->server_ctx;
    i2c_txn_t* txn = get_i2c_txn(dev->bus, req, channel);
    if (!txn) {
        return ZX_ERR_NO_MEMORY;
    }

    return i2c_transact(i2c_channel, data, req->txn_ctx.write_length, req->txn_ctx.read_length,
                        platform_i2c_complete, txn);
}
...
static zx_status_t platform_dev_rxrpc(void* ctx, zx_handle_t channel) {
    ...
    case PDEV_I2C_TRANSACT:
        resp.status = platform_i2c_transact(dev, &req->i2c, &resp.i2c, req_data.data, channel);
        break;
